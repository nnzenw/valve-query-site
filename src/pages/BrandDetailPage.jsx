...
export default function BrandDetailPage({ brandId }) {
  ...
  const loadProducts = async (pageNum = 1, isPageChange = false) => {
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/valves/search?brand_id=${brandId}&limit=${PAGE_SIZE}&offset=${offset}`)
      if (res.ok) {
        const data = await res.json()
        setValves(data.data || [])
        setTotalCount(data.count || 0)
        if (!brandInfo && data.data?.length > 0 && data.data[0].brands) {
          setBrandInfo(data.data[0].brands)
        }
      }
    } catch (err) {
